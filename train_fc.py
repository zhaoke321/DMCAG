# ============================================================
# [DUPLICATE] 本段代码在 train_fc.py / train_conv_mnist-usps.py /
#   train_conv_multi-coil-10.py 中完全相同或高度相似（仅 tqdm 导入不同）
# 包含：imports, os.environ, min_max_scaler, normalize, cpu 线程设置
# 重复行数：约 30 行 × 3 文件 ≈ 90 行
# ============================================================
...
import tqdm  # train_fc.py 独有，另两个文件无此行
warnings.filterwarnings('ignore')
# [DUPLICATE-BUG] 以下两行重复定义（参见本文件第 27-28 行）
...
# [DUPLICATE] ClusteringLayer: 3 个训练文件中完全相同
# 出现在 train_fc.py / train_conv_mnist-usps.py / train_conv_multi-coil-10.py
# 重复行数：11 行 × 3 文件 ≈ 33 行（建议移入 utils.py）
...
class SingleViewModel(nn.Module):
    ...
    # [DUPLICATE] computeA: 3 个训练文件中完全相同
    ...
    # [DUPLICATE] computegcn: 3 个训练文件中完全相同（本文件还有独立的同名函数）
...
# ============================================================
# [DUPLICATE BLOCK START] 以下 8 个函数在 3 个训练文件中完全相同
# computegcn, make_qp, target_distribution, graph_fusion,
# calculate_c, quadprog, cacluate_U, mask_correlated_samples,
# embeddingcontras, labelcontras, setup_seed
# 重复行数：约 170 行 × 3 文件 ≈ 510 行
# ============================================================

# [DUPLICATE] 与 SingleViewModel.computegcn 完全相同的独立函数版本
def computegcn(z, Af):
    D = torch.sum(Af, dim=1)
    D = 1 / D
    D = torch.sqrt(D)
...
def cacluate_U(C):
    ...
    return U
# ============================================================
# [DUPLICATE BLOCK END] 以上函数在所有 3 个训练文件中完全相同
# ============================================================

# [PARTIAL-DUPLICATE] pretrain_aes: 3 个训练文件 ~70% 相同
# 差异仅在：数据集类、tqdm 使用、评估代码注释/取消注释、batch_size
def pretrain_aes():
...
# ============================================================
# [DUPLICATE BLOCK START] 对比学习相关函数，3 个训练文件完全相同
# ============================================================
...
def labelcontras( q_i, q_j):
    ...
    return loss + entropy
# ============================================================
# [DUPLICATE BLOCK END] 对比学习函数
# ============================================================

# [PARTIAL-DUPLICATE] fineTuning: 3 个训练文件 ~70% 相同
# 差异仅在：view_loss 权重 (0.1 vs 0 vs 无)、tqdm 使用、epoch 边界条件、
# 最终预测方法 (KMeans vs argmax(qpred))
def fineTuning():
    model = MultiViewModel(
        n_enc_1=500,
        n_enc_2=500,
...
# [PARTIAL-DUPLICATE] argparse + main: 3 个训练文件 ~80% 相同
# 差异仅在：数据集默认值、save_path、数据集配置参数
